...
string barcode_A="../data/BC_A.txt";
string barcode_B="../data/BC_B.txt";
string barcode_C="../data/BC_C.txt";
  ...
  if(line.length()<code_total_length){
    ...
    if(a==bc1.end()){
      codeA=min_edit_distance(codeA_inFile,bc1,code_letter1);
			read_type1="corrected";   // ??? forced read_type
    }else
      codeA=a->second;
...
int main (int argc, char* argv[])
{
  ...
  string path_to_reads=(argc > 1) ? argv[1] : string("../data/_gen_");
  string path_output=(argc > 2) ? argv[2] : string("../data/");
